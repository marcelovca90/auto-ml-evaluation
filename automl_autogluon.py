...
####################################################################################
# https://auto.gluon.ai/0.3.1/tutorials/tabular_prediction/tabular-multilabel.html #
####################################################################################
    ...
    for SEED in PRIME_NUMBERS:
        ...
        try:
            ...
            if is_multi_label() and not LABEL_POWERSET:
                train_df = pd.concat([X_train, y_train], axis='columns').dropna()
                test_df = pd.concat([X_test, y_test], axis='columns').dropna()
                labels = list(set(y_train.columns) & set(y_test.columns))
                problem_types = ['binary'] * len(labels)
                eval_metrics = ['f1_weighted'] * len(labels)
                path = 'Autogluon-' + str(uuid.uuid4())
                exec_time_seconds_actual = EXEC_TIME_SECONDS // len(labels)
                clf = MultilabelPredictor(labels=labels, problem_types=problem_types, eval_metrics=eval_metrics, path=path)
            else:
                ...
                exec_time_seconds_actual = EXEC_TIME_SECONDS
                y_test = test_df['class'].values
                clf = TabularPredictor(problem_type=infer_task_type(y_test), eval_metric='f1_weighted', label='class')

            TIMER.tic()
            clf.fit(time_limit=exec_time_seconds_actual, train_data=train_df, num_cpus=NUM_CPUS)
